...
public class Computer extends Calculator{
    ...
    //오버라이딩을 하면 자식클래스 전용으로 만듬.
    //접근제한자, 반환타입, 메소드명, 매개변수의 타입, 순서, 개수 모두 부모 클래스에서
    //상속받은 그대로 사용해야함.
    ...
    //자식 클래스에서 오버라이딩 시 @Override 어노테이션을 반드시 사용할 필요는 없다.
    //@Override 어노테이션 없어도 부모 클래스에서 상속받은 메소드와 동일한 메소드가 있을 경우 오버라이딩이 됨.
    //@Override 어노테이션이 있으면 컴파일러가 해당 메소드를 오버라이딩된 메소드로 인식하기 떄문에 규칙을 한번 더 확인함.
